...
void UDCoreBoss::Run(void)
{
  ...
  switch (o->state)
  {
    ...
    case CR_FaceDoom + 1:
    {
      ...
      if (o->timer3 == 75)
      {
        CreateObject(MAPX(map.xsize) + 40, MAPY(3 + random(-3, 0)), OBJ_UDMINI_PLATFORM);
      }
      ...
      {
        o->timer3 = 0;
        CreateObject(MAPX(map.xsize) + 40, MAPY(10 + random(-1, 3)), OBJ_UDMINI_PLATFORM);
...
void ai_ud_pellet(Object *o)
{
  switch (o->state)
  {
    ...
    case 3:
    {
      ...
      if (o->CenterX() < (16 * CSFI) || o->CenterX() > (MAPX(map.xsize) - (16 * CSFI)))
      {
        o->Delete();
      }
    }
...
void ai_ud_blast(Object *o)
{
  ...
  SmokePuff(o->CenterX() + (random(0, 16) * CSFI), o->CenterY() + (random(-16, 16) * CSFI));
